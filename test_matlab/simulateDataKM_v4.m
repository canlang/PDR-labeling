...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
loc     = A.data(1,:);
theta   = -90;      %% INIT HEADING
hDelta  = 0;        %% HEADING DRIFT CONTROL
step    = 0.5;
turn    = 0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% LOCATION HISTORY
...
f = figure('visible','on');
clf(f);
set(f,'position',[100,100, 1700,1000]);

subplot(2,2,3)
rowSize = size(R,1);
for j = 1:rowSize
    line(R(j,[1,3]), R(j,[2,4]),...
...
%             circle_n1(loc(2),loc(3),10, theta,'g');
            ...
            if all(ismember([A.textdata{startIdx:i,1}], 'A') == 0) && turn
                ...
                disp (['heading end. SUM of heading delta: ',num2str(hwindow(1)-hwindow(end))]);
                
                if abs(hwindow(1)-hwindow(end)) > 70
                    disp ('TURN DETECT!')
                    ...
                    errfun = @(x)turnPointErr4(x(1),x(2),locHis,turnTime,12000,NN_1,R);
...
%                     optVals = fminunc(errfun,[1,0],options);
                    ...
                    [x,y] = meshgrid(0:.1:2, -25:25);
                    errfun = @(x,y)turnPointErr4(x,y,locHis,turnTime,12000,NN_1,R);
