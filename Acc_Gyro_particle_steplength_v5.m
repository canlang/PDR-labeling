...
if (strcmp('190409_172013_390_N1_upper_corridor_rover',target_rawdata_paths{j}) == 0)  
    continue
else
    fprintf ( 1, 'find index : %d\n',j );
end
...
init = [mean([7.46,10.89]),mean([22.38,19.25])]; % for highres N1 map

% reference : end point info.
% end_point = [12.8,23.98];
% end_point = [74,20];
% end_point = [mean([42.29,51.23]),mean([12.24,17.09])];
end_point = [mean([7.46,10.89]),mean([22.38,19.25])]; % for highres N1 map
...
if video_flag
    v = VideoWriter(...
        fullfile('output',strcat(target_rawdata_paths{j},...
        '.mp4')),'MPEG-4');
...
%     phy_move_noise_range = .1;
    ...
    if exist('d_yaw','var') == 1
        % resampling noise: step length
        ps.sl = ps.sl(resample_idx)+random('normal',0,std_euler(time_i),[n,1]);
        % resampling noise: heading
%         noise_alpha = std_euler(time_i);
        noise_alpha = abs(d_yaw);
%         ps.init_heading = ps.init_heading(resample_idx)+noise_alpha*random('normal',0,.01,[n,1]);
...
%     ps.init_heading = ps.init_heading(resample_idx)+random('normal',0,.1,[n,1]);    
        ...
        ps.init_heading = ps.init_heading(resample_idx);
    end
...
% visusalize result
hold on
est_p = plot(ps.hist_x(k_idx,:),ps.hist_y(k_idx,:),'-.r*');
% plot(ps.hist_x(k_idx,:),ps.hist_y(k_idx,:),'-mo',...
%     'LineWidth',2,...
%     'MarkerEdgeColor','k',...
%     'MarkerFaceColor',[.49 1 .63],...
%     'MarkerSize',10);
text(1,3,sprintf('Penalty score = %.2f\n',(1/min(k_d))));
hold off

legend(est_p,sprintf('Estimated:\nbest fit trajectory'),'Location','east');
% legend([est_p],'Estimated path (Best fit)') % appliable multiple ends
...
% file write
dlmwrite(fullfile('output',strcat(target_rawdata_paths{j},'.csv')),trace_rs,'precision', '%8.2f');
...
% save result image
print(fullfile('output',target_rawdata_paths{j}),'-dpng')
% ,'-r300')
