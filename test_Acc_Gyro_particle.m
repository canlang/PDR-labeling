...
% 3. initial area
ps.x = 23+random('normal',0,.5,n,1);
ps.y = 11+random('normal',0,.5,n,1);
ps.sl = .7+random('normal',-.1,.1,n,1);
...
%     v = VideoWriter('vids/pdr_particle_labeling_.mp4','MPEG-4');
    v = VideoWriter('vids/pf_labeling_sl_0.1_2.avi','Motion JPEG AVI');
    v.FrameRate = 10;
    v.Quality = 100;
    open(v);
    frame = getframe(gcf);
...
for i=1:length(locs)
    time_i = locs(i);
    yaw = euler(time_i,3);
%     step_length = .6+random('normal',-.1,.1,n,1);
    ...
    ps.init_heading = ps.init_heading(resample_idx)+random('normal',.01,.01,n,1);
